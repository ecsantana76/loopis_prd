...
const mockOrders: Record<string, Order> = {
  'LOOP-8821': {
    ...
    totalPayable: 241.20, // (268 - 53.60) + 26.80
    items: [
      {
        id: 'it_1',
...
export const mockApi = {
  /**
   * Fetch and parse an order receipt by QR Code / barcode string or typed code
   */
  async fetchOrderReceipt(rawCode: string): Promise<Order> {
    await delay(750);
    const normalized = rawCode.trim().toUpperCase();

    // Specific error simulation cases
    ...
    // Direct lookup or fallback to default Sintra order if recognized pattern
    if (mockOrders[normalized]) {
      return JSON.parse(JSON.stringify(mockOrders[normalized]));
    }

    // If matches format LOOP-XXXX or CMD-XXXX generate dynamic realistic comanda
  ...
  /**
   * Search registered Loopis users for splitting bills
   */
  ...
  /**
   * Create or update a split payment session
   */
  ...
  /**
   * Retrieve active split session by ID
   */
  ...
  /**
   * Mark a specific participant as paid in real-time
   */
  ...
  /**
   * Fetch restaurant commercial settings
   */
  ...
  /**
   * Save restaurant commercial settings with schema validation
   */
  ...
  /**
   * Calculate live commercial projection
   */
  calculateCommercialProjection(simulatedGmv: number, settings: RestaurantBillingSettings): CommercialSimulationResult {
    ...
    // Customer cashback estimate based on conversion
    const estimatedCustomerCashbackLoops = settings.acceptsLoopCredits
      ? Math.round(simulatedGmv * (settings.loopConversionRate || 1.0) * 0.1) // 10% of spend converted to loops
      : 0;
