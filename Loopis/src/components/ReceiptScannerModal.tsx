...
export const ReceiptScannerModal: React.FC<ReceiptScannerModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleGoToSplit = () => {
    ...
    onClose();
    navigate(`/checkout/split?orderCode=${scannedReceipt.orderCode}&netTotal=${scannedReceipt.totalAmount}`);
  };
    ...
    <AnimatePresence>
      <div className="partner-modal-backdrop fixed inset-0 z-[100] flex items-end justify-center overflow-hidden p-0 sm:items-center sm:p-6">
        ...
        <motion.div
          ...
          className="partner-modal-panel relative z-10 flex w-full max-w-lg flex-col overflow-hidden rounded-t-3xl border border-gray-100 bg-white shadow-2xl dark:border-white/10 dark:bg-[#1a1a1c] sm:rounded-3xl"
        >
