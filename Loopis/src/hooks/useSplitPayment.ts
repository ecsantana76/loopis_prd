...
export const useSplitPayment = ({
  ...
  const [activeTab, setActiveTab] = useState<'full' | 'split'>('split');
  ...
  const handlePayFullDirect = () => {
    ...
    const earnedLoops = Math.round(totalAmount);
    useStore.setState((state) => ({
      loopsBalance: state.loopsBalance + earnedLoops,
      transactions: [
        {
          ...
          amount: earnedLoops,
          type: 'earn',
          description: `Pagamento Integral: Comanda ${orderCode} (${restaurantName})`
        },
        ...state.transactions
      ]
    }));
    triggerConfetti();
    setIsCompletedModalOpen(true);
    showToast('Comanda integral paga com sucesso!');
  };
